tableextension 50109 tableextension50109 extends "Sales Invoice Header" 
{
  ...
  fields
  {
    modify("Invoice Type")
    {
      OptionCaptionML = ENU='F1 Invoice,F2 Simplified Invoice,F3 Invoice issued to replace simplified invoices,F4 Invoice summary entry',
                        ESP='F1 Invoice,F2 Simplified Invoice,F3 Invoice issued to replace simplified invoices,F4 Invoice summary entry';
    }
    ...
    {
      CalcFormula=Sum("Sales Invoice Line"."Income Tax Base" where ("Document No."=FIELD("No."),
